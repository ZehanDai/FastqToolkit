        ...
        while True:
            ...
            while True:
                ...
                if line.startswith('@'):
                    ...
                    if error_log_handle:
                        error_log_handle.write(f"SKIP_UNEXPECTED\t{filepath}\tline {line_num}\t{line[:80].rstrip()}\n")
            # Read three more lines
            seq = f.readline()
            if not seq:
                if error_log_handle:
                    error_log_handle.write(f"TRUNCATED\t{filepath}\tline {line_num}\tmissing sequence\n")
                continue
            seq = seq.rstrip('\n')
            plus = f.readline()
            if not plus or not plus.startswith('+'):
                if error_log_handle:
                    error_log_handle.write(f"TRUNCATED\t{filepath}\tline {line_num}\tmissing/invalid '+'\n")
                continue
            plus = plus.rstrip('\n')
            qual = f.readline()
            if not qual:
                if error_log_handle:
                    error_log_handle.write(f"TRUNCATED\t{filepath}\tline {line_num}\tmissing quality\n")
                continue
            qual = qual.rstrip('\n')
            ...
            if read_id is None:
                if error_log_handle:
                    error_log_handle.write(f"INVALID_HEADER\t{filepath}\tline {line_num}\t{header[:80]}\n")
                continue
...
def filter_fastq(input_path, output_path, keep_ids,  error_log):
    ...
    If new_run_number is not None, modify the header's run number.
    """
...
def get_new_run_number(original_run, mode, seed=None):
    """Return the new run number based on mode."""
    if mode == 'keep':
        return None
    if mode == 'half':
        return original_run // 2
    if mode == 'random':
        low = int(original_run / 3) + 1
        high = int(original_run * 2 / 3)
        if low > high:
            low = high = original_run // 2
    ...
    if mode.startswith('fixed:'):
        try:
            ...
            raise ValueError(f"Invalid fixed value in {mode}")
    raise ValueError(f"Unknown runid mode: {mode}")

def main():
    parser = argparse.ArgumentParser(description="Extract random subset of paired-end FASTQ reads with optional run ID modification.")
