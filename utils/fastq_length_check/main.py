...
# ============================================================
# Step 4b: length distribution table & histogram
# ============================================================

def _scale_bar(count, max_count, width=40):
    """Return a string of block characters proportional to count/max_count."""
    if max_count <= 0:
        return ""
    n = int(round(count / max_count * width))
    return "█" * max(1, n) if count > 0 else ""


def write_length_distribution(r1_dist, r2_dist, out_prefix, top_n=None):
    """
    Write length-distribution outputs:

      <out_prefix>.tsv — frequency table
          columns: length  R1_count  R1_percent  R2_count  R2_percent

      <out_prefix>.txt — ASCII histograms for R1 and R2

    If top_n is given, only the top N most frequent lengths are shown
    in the histogram (the TSV always contains all lengths).
    """
    tsv_path = f"{out_prefix}.tsv"
    txt_path = f"{out_prefix}.txt"

    all_lengths = sorted(set(r1_dist) | set(r2_dist))
    total_r1 = sum(r1_dist.values()) or 1
    total_r2 = sum(r2_dist.values()) or 1

    # ---- TSV frequency table ----
    with open(tsv_path, 'w') as fo:
        fo.write("length\tR1_count\tR1_percent\tR2_count\tR2_percent\n")
        for L in all_lengths:
            c1 = r1_dist.get(L, 0)
            c2 = r2_dist.get(L, 0)
            fo.write(
                f"{L}\t{c1}\t{100 * c1 / total_r1:.4f}\t"
                f"{c2}\t{100 * c2 / total_r2:.4f}\n"
            )

    # ---- ASCII histograms ----
    def _hist_lines(dist, total, label, top_n):
        items = sorted(dist.items())          # by length ascending
        if top_n:
            # keep the top-N most frequent lengths, then re-sort by length
            top = sorted(dist.items(), key=lambda x: -x[1])[:top_n]
            keep = {k for k, _ in top}
            items = [(L, c) for L, c in items if L in keep]
        max_count = max(dist.values()) if dist else 0

        lines = [
            f"--- {label} length distribution ---",
            f"{'length':>8}  {'count':>12}  {'percent':>9}  {label}",
        ]
        for L, c in items:
            pct = 100 * c / total
            bar = _scale_bar(c, max_count)
            lines.append(f"{L:>8}  {c:>12,}  {pct:>8.4f}%  {bar}")
        return lines

    with open(txt_path, 'w') as fo:
        fo.write("=== LENGTH DISTRIBUTION ===\n\n")
        fo.write("\n".join(_hist_lines(r1_dist, total_r1, "R1", top_n)))
        fo.write("\n\n")
        fo.write("\n".join(_hist_lines(r2_dist, total_r2, "R2", top_n)))
        fo.write("\n")

    return tsv_path, txt_path


def write_png_histogram(r1_dist, r2_dist, out_path):
    """
    Optional PNG histogram. Requires matplotlib.
    ...
    try:
        ...
        print("matplotlib not installed, skipping PNG histogram", file=sys.stderr)
        return False

    fig, axes = plt.subplots(2, 1, figsize=(10, 6), sharex=True)

    for ax, dist, label in zip(axes, [r1_dist, r2_dist], ["R1", "R2"]):
        lengths = sorted(dist.keys())
        counts = [dist[L] for L in lengths]
        ax.bar(lengths, counts, width=0.9, align="center")
        ax.set_ylabel("count")
        ax.set_title(f"{label} length distribution")
...
def main():
    ...
    parser.add_argument("--length-report", default=None, metavar="PREFIX",
                        help="Write length distribution table + ASCII histogram "
                             "to PREFIX.tsv / PREFIX.txt")
    parser.add_argument("--length-png", action="store_true",
                        help="Also write PREFIX.png histogram (requires matplotlib)")
    parser.add_argument("--length-top", type=int, default=None,
                        help="Show only the top N most frequent lengths in the "
                             "histogram (TSV keeps all lengths)")
    ...
    # ---- Step 2a: length distribution table & histogram ------------------
    if args.length_report:
        print(f"Writing length distribution to {args.length_report}.* ...",
              file=sys.stderr)
        tsv_path, txt_path = write_length_distribution(
            r1_dist, r2_dist, args.length_report, top_n=args.length_top
        )
        print(f"  Table: {tsv_path}", file=sys.stderr)
        print(f"  Histogram: {txt_path}", file=sys.stderr)
        if args.length_png:
            png_path = f"{args.length_report}.png"
            if write_png_histogram(r1_dist, r2_dist, png_path):
                print(f"  PNG: {png_path}", file=sys.stderr)

    # ---- Step 2b: compute thresholds --------------------------------------
